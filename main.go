...
func convertPrecoFloat(s1, s2 string) (vlrAnuncio float64, err error) {
	if s2 == "" {
		s2 = "00"
	}
	...
	return 0.0, errors.New("Não foi possível converter as STRINGS " + s1 + " e " + s2 + " do preço para um FLOAT")
}
...
func tratarQtdResultados(resultados string) (qtdResult int64, err error) {
	...
	if err != nil {
		return 0, fmt.Errorf("erro ao tratar a string [%s] para coletar somente o numero -- Erro [%w]", resultados, err)
	}
	return x, nil
}
...
func (a *anuncioML) GetPatrocinado() {
...
func (a *anuncioML) GetFull(prod colly.HTMLElement) {
...
func (a *anuncioML) GetRating(prod colly.HTMLElement) {
...
// Por isso a função constrói o valor primeiro pegando o FRACTION e depois checando a existencia do cents, criando a string e tratando ela com a função convertPrecoFloat()
func (a *anuncioML) GetPrecoAtual(prod colly.HTMLElement) {
	...
	if checkprecoAtualCentavos := prod.DOM.Find(".ui-pdp-price__second-line span.andes-money-amount__cents").First().Text(); len(checkprecoAtualCentavos) != 0 { //caso tenha centavos
		precoAtualConvertido, err := convertPrecoFloat(precoAtual, checkprecoAtualCentavos)
		if err != nil {
			fmt.Println(err)
		} else {
		...
		a.precoAtual, _ = convertPrecoFloat(precoAtual, "")
	}
}
...
// Valor é "construido" na pagina do ML por 2 elementos: MONEY-AMOUNT_FRACTION e MONEY-AMOUNT_CENTS. Se for um preço "cheio", não tem o cents.
// Por isso a função constrói o valor primeiro pegando o FRACTION e depois checando a existencia do cents, criando a string e tratando ela com a função convertPrecoFloat()
func (a *anuncioML) GetPrecoBase(prod colly.HTMLElement) {
	if checkPrecoBase := prod.DOM.Find(".ui-pdp-price__original-value span.andes-money-amount__fraction").First().Text(); len(checkPrecoBase) != 0 {
		precoBaseConvertido, err := convertPrecoFloat(checkPrecoBase, prod.DOM.Find(".ui-pdp-price__original-value span.andes-money-amount__cents").First().Text())
		if err != nil {
			fmt.Println(err)
		} else {
...
func (a *anuncioML) GetVendas(prod colly.HTMLElement) {
...
func (a *anuncioML) GetVendedorLink(prod colly.HTMLElement) {
...
func (a *anuncioML) GetEstoque(prod colly.HTMLElement) {
	...
	if len(estoqueUltimo) != 0 && estoqueUltimo == ultimoEstoque {
		...
		estoqueNaoUltimo = estoqueNaoUltimo[1 : len(estoqueNaoUltimo)-1]
...
func (a *anuncioML) GetDescricao(prod colly.HTMLElement) {
	a.descricao = prod.ChildText(".ui-pdp-description__content")
}

// TODO: quando futuramente virar um executavel e o parametro de pesquisa ser digitado pelo usuario e não passado como uma flag de execução, trocar para esse input do usuario
...
func main() {
	...
	for {
		fmt.Printf("Digite a sua pesquisa para gerar um csv com os dados dos anúncios buscados nos marketplaces:\n")
	...
	scrapperDetalhado.OnScraped(func(r *colly.Response) {
		fmt.Printf("Dados coletados com sucesso...\n")
	...
	scrapper.OnHTML("body main div.ui-search", func(e *colly.HTMLElement) {
		...
		s, _ := tratarQtdResultados(qtdResultados)
	...
	scrapperDetalhado.OnHTML("body main", func(prod *colly.HTMLElement) {
		...
		anuncio.GetPatrocinado()
		anuncio.GetFrete(*prod)
		anuncio.GetFull(*prod)
		anuncio.GetRating(*prod)
		anuncio.GetQtdReview(*prod)
		anuncio.GetPrecoAtual(*prod)
		anuncio.GetPrecoBase(*prod)
		anuncio.GetVendas(*prod)
		anuncio.GetVendedor(*prod)
		anuncio.GetVendedorLink(*prod)
		anuncio.GetEstoque(*prod)
		anuncio.GetDescricao(*prod)
	...
	if err != nil {
		fmt.Printf("Erro ao criar e escrever dados para o csv [%v]\n", err)
	} else {
